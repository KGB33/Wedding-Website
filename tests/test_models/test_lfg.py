...
@pytest.fixture
def lfg():
    return LFG("Owner", ["Member1", "Member2", "Member3"], 6, "Info Blob", "HOTEL")

@pytest.mark.no_mongo_db
class TestInit:

    def test_valid_init(self):
        lfg = LFG("Owner", ["Member1", "Member2", "Member3"], 6, "Info Blob", "HOTEL")
        assert lfg.owner == "Owner"
    ...
    def test_too_many_members(self):
        with pytest.raises(ValueError) as excinfo:
            lfg = LFG("Owner", ["Member1", "Member2", "Member3"], 2, "Info Blob", "HOTEL")
...
class TestStr:
    ...
    def test_empty_members(self, lfg):
        lfg.members = []
        assert "LFG is owned by Owner and has 1/6 members" == str(lfg)
    
    def test_with_members(self, lfg):
        "LFG is owned by Owner and has 4/6 members" == str(lfg)
